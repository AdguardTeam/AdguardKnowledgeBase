...
Например, правило `/banner\d+/$third-party` следующее правило применит регулярное выражение `banner\d+` ко всем сторонним запросам.
...
Правило применяется только к ajax-запросам. То есть к запросам, отправленным с помощью `XMLHttpRequest`.
