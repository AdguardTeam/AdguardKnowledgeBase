...
Различные версии Firefox требуют разных подходов для осуществления [HTTPS-фильтрации](https://kb.adguard.com/ru/general/https-filtering). Как правило, проблема установки пользовательского сертификата возникает до релизной версии Firefox 81.1.1. Для решения воспользуйтесь Методом 1, описанным ниже. Он также подходит для nightly- и beta- версии браузера.

### Предоставление доступа к сертификатам

#### Метод 1: Сертификат находится в **пользовательском хранилище**

Чтобы начать использовать пользовательские сертификаты в **последней стабильной** версии, убедитесь, что вы обновлены до 81.1.1 и новее. Для неё, beta и nightly версии (и браузерах на основе Firefox) вам необходимо активировать скрытую опцию в настройках браузера:
