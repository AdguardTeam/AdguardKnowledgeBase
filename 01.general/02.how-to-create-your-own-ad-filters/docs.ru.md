...
Чтобы сопоставление целевого домена было выполнено, необходимо соблюсти следующее условие: `1 и ((2 и 3) или 4)`. То есть, если модификатор `$domain` содержит только исключённые домены, то правилу не нужно выполнять второе и третье условия, чтобы сопоставить целевой домен с модификатором `$domain`.
