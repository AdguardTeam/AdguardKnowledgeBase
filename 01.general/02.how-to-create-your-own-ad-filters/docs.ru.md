...
В некоторых случаях может потребоваться отключение всех запрещающих правил. Например, на время тестирования. Для этого следует воспользоваться правилом исключения без указания домена. Оно полностью отключить соответствующее CSS/elemhide-правило на ВСЕХ доменах:

```
#@#.textad
```

То же самое может быть достигнуто добавлением следующего правила:
```
*#@#.textad
```
