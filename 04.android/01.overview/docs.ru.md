...
Но у режима совместимости есть ряд ограничений. В нём не будут работать:
* DNS
* Модуль *Настройки приложений* (ранее заданные настройки не слетят, но и не будут применяться)
* Kill Switch (функция AdGuard VPN)
* Автозащиту (функция AdGuard VPN)
