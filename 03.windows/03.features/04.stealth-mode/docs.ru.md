...
Значение заголовка Authorization - это просто base64-кодированная версия учетных данных (<username>:<password>), которые отправляются на сервер для проверки подлинности. В этом случае данные будут доступны любому, кто сможет перехватить HTTP-запрос. Включение этой опции приведет к удалению заголовков Authorization для всех сторонних запросов. Обратите внимание, что это может привести к необходимости повторного ввода учетных данных в веб-формах.
