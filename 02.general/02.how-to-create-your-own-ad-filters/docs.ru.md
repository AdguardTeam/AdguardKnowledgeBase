...
```
Это правило применяет скриптлет abort-on-property-read ко всем страницам example.org и его поддоменов, а также передаёт ему один аргумент (alert).
...
AdGuard использует тот же синтаксис фильтрации, что и uBlock Origin. Кроме того, он совместим с модификатором `$rewrite` AdblockPlus.

`$redirect` — это модификатор для базовых правил фильтрации, так что правила с этим модифиактором поддерживают все другие базовые модификаторы, такие, как `$domain`, `$third-party`, `$script` и т.д.
...
```
Это правило перенаправляет все запросы к test.mp4 на ресурс с названием noopmp4-1s.

>Дополнительная информация о сркиптлетах, редиректах и их использовании доступна по [этой ссылке](https://github.com/AdguardTeam/Scriptlets#adguard-scriptlets-and-redirect-resources).
