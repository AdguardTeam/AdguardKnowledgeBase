...
Правила с модификатором `$removeparam` предназначены для того, чтобы убирать параметры из URL страниц. Обратите внимание, что такие правила применяются толькок запросам типов `GET`, `HEAD`, и `OPTIONS`.

>**Совместимость с различными версиями AdGuard**
>Правила с модификатором `$removeparam` поддерживаются приложениями AdGuard для Windows, Mac, и Android.

>**Ограничения**
>Пожалуйста, заметьте, что этот тип правил может быть использован **только в доверенных фильтрах**. Эта категория включает в себя ваш собственный **Пользовательский фильтр**, а также все фильтры, созданные командой разработчиков AdGuard.
...
Этот тип правил работает практичски так же, как и в случае с модификаторами [`$csp`](#csp-modifier) и [`$redirect`](#redirect-modifier).
...
>В случае, когда несколько правил с `$removeparam` соответствуют одному запросу, они все будут применены одно за одним.
...
При помощи этих правил указанные UTM-параметры будут убраны из любого запроса, кроме запросов к example.org.

>**Совместимость с другими модификаторами**
>Правила `$removeparam` не совместимы с любыми другими модификаторами, кроме `$domain`, `$third-party`, `$app`, `$important` и `$match-case`. Правила, имеющие любые другие модификаторы, будут считаться некорректными и не будут применяться.

>Пожалуйста, обратите внимание на то, что правила с `$removeparam` могут также быть отключены правилами-исключениями `$document` и `$urlblock`. Но базовые правила-исключения без модификаторов не будут этого делать. Например, `@@||example.com^` не отключит `$removeparam=p` для запросов к **example.com**, в то время как `@@||example.com^$urlblock` отключит.
...
> **Обратите внимание**, что блокирующее правило с параметром `$removeparam` должно содержать хотя бы один уточнённый параметр. Например, правило вроде `example.com$removeparam` будет считаться неверным и применяться не будет. 
