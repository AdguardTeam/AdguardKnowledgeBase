...
> #### Совместимость с разными версиями AdGuard
> Модуль "Антитрекинг" на данный момент доступен в приложениях AdGuard для Windows, AdGuard для Mac и AdGuard для Android. В будущем мы планируем добавить его во все продукты AdGuard. В продуктах без поддержки модуля "Антитрекинг" правила с этим модификатором игнорируются.
...
### Нюансы применения
Я полагаю, этого достаточно для перехвата `Cookie`/`Set-Cookie` заголовков и нет необходимости использовать свойство `document.cookie`.

Давайте рассмотрим пример.

1. `||example.org^$cookie=i_track_u` правило должно блокировать `i_track_u` cookie от `example.org`
2. Мы перехватываем запрос отправленный к `https://example.org/count`
3. Значение `Cookie` заголовка `i_track_u=1; JSESSIONID=321321`
4. В первую очередь, изменяем `Cookie` заголовок, чтобы сервер не получил значение `i_track_u`. Измененное значение: `JSESSIONID=321321`
5. Ждем ответ и проверяем все `Set-Cookie` заголовки, полученные с сервера.
6. Удаляем те, которые устанавливают cookie `i_track_u` (или изменяем те cookie, которые содержат более одного)
7. Теперь нам надо убедиться, что браузер удаляет эти куки. Для этого нам надо добавить новый `Set-Cookie` заголовок, который устанавливает `i_track_u` с отрицательным сроком хранения: `Set-Cookie: i_track_u=1; expires=[CURRENT_DATETIME]; path=/; domain=.example.org`.

> **Шаг 7 не должен выполняться, если правило имеет `third-party` модификатор**. `third-party` означает, что могут быть случаи (`first-party`), когда cookie не должны удаляться, т.е. они могут быть полезными и удаление их может повлиять на нормальную работу сайта. Например, Google и Facebook полагаются на свои SSO cookie и их принудительное удаление выведет вас из системы (разлогинит).
