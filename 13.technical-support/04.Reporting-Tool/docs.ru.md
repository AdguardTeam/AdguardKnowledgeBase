...
2. Если это решило проблему, включите модуль и поочерёдно отключайте фильтры с проверкой результата. Это необходимо для того, чтобы определить какие фильтры приводят к некорректному отображению страницы;
...
3. Если вам удалось выявить какая настройка приводит к некорректной работе страницы, вам необходимо сообщить об этом разработчикам при заполнении формы (см. выше). В этом случае, желательно сбросить настройки в исходное состояние;
