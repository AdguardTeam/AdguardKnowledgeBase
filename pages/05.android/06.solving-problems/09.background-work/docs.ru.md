...
# Если AdGuard через некоторое время исчезает из фоновой работы
В некоторых случаях приложения не будут оставаться в фоновом режиме из-за функции оптимизации ОС Android или из-за режима энергосбережения. Чтобы избежать этого вам необходимо выполнить следующие шаги указанные для каждой версии ОС Android. * (Обратите внимание, что эта инструкция может быть применима практически для каждой версии Android OS) * ._
...
- Нажмите на *кнопку-меню фоновых приложений* и проведите по экрану AdGuard, чтобы сделать опции * видимыми * (как показано на снимке экрана):
