...
Браузерный Помощник – это нативное расширение (не путать со старым Помощником для Mac, который существовал до версии AdGuard 2.4), которое одновременно берёт на себя роли старого Помощника и Режима интеграции. В [Режиме интеграции](https://kb.adguard.com/ru/browser-extensions/integration-mode) нагрузка распределялась между браузерным расширением и программой для Mac, но теперь один Браузерный Помощник справляется с задачей лучше.

<img src="https://cdn.adguard.com/public/Adguard/Release_notes/Mac/v2.4/safari_assistant_pop-up_menu_ru.png" />

Прямо из настроек Браузерного Помощника можно управлять десктопной программой AdGuard для Mac: запускать её, если она не работает, включать/выключать защиту в один клик и получать быстрый доступ к настройкам или журналу фильтрации прямо из браузера. Ещё одна полезная функция – это отображение реального сертификата отфильтрованной страницы.
...
Фильтрации сети посвящено последнее окно, на нём вы найдёте расширенные функции. Рекомендуем держать включенными первые две опции: «Автоматически фильтровать трафик приложений» и «Фильтровать HTTPS-протокол» — по сути это основная защита для вашего интернета. HTTP — это основной протокол передачи информации через интернет. По нему передается большинство данных, включая рекламу, но у него есть один недостаток — соединение не зашифровано. Благодаря функции «Фильтровать HTTPS-протокол», фильтруется не только HTTP-трафик, но и HTTPS-трафик (где S означает secure, то есть "безопасный").
