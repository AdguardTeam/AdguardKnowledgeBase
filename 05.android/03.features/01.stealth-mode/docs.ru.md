...
Когда вы посещаете веб-сайт, ваш браузер отправляет серверную информацию о себе. Эта информация отправляется в виде текстовой строки со слов "User-Agent:" и является частью HTTP-запроса. Обычно она включает в себя имя и версию браузера, а также языковые настройки. 
