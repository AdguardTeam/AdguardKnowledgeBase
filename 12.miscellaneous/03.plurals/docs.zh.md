...
众所周知，世界上的语言之间存在很多差异。这些差异之一是使用复数形式，翻译成不同语言可能会造成巨大困难。

因此，我们强烈建议您正确阅读本文，以详细了解复数形式以及翻译AdGuard时如何使用它们。
...
与英语相反，波兰语具有三种复数形式的名词。在谈论单数形式时使用一种形式，在复数形式中使用另一种形式，但数字以 2-4 结尾，但不包括 12-14，第三种形式-前面带有其他数字的单词。
...
**如果我们忽略其中一种形式，而只为斯洛伐克语使用两个模板，而不是三个，则系统将无法为某些数字采用适当的模板**。结果，句子中将出现语法错误，例如英语：*Standard license for 5 computer*。

但是在某些语言中，可能有一些单词(简称为排除词)的复数形式少于语言本身。这可能会引起一些混乱。

例如，塞尔维亚单词 *'računar'*（计算机(computer)）只有两种形式：*'računar'* -表示1、21、31（以及其他以1结尾的数字，不包括11）计算机(computers)和 *'računara'* -用于其他数量的计算机。 但是尽管如此，但根据一般规则（说明塞尔维亚语中的单词有三种复数形式），AdGuard翻译中应该有三个模板：
