...
Разработчики рекламы тоже не сидят на месте и постоянно меняют способы внедрения рекламы. Сайт обнаруживает, что реклама была удалена, и вставляет её обратно при помощи зашифрованных скриптов. Как правило, разработчики подобных сайтов постоянно тестируют новые алгоритмы на определённой группе пользователей, при этом все остальные получают рекламу по старым алгоритмам, для которых мы подобрали решение. Это очень затрудняет поиск причины, поскольку данная реклама не отображается у разработчиков. Вы можете помочь нам заполнив заявку и дополнив её примерами рекламы, используя инструкции в зависимости от вашей ОС.

## Отправка жалобы

Чтобы отправить жалобу на повторную вставку рекламы или некорректное отображение сайта, вам необходимо:

> ### При отправке жалобы через приложение или Помощник AdGuard вся информация (кроме скриншотов, их нужно будет сделать и прикрепить самостоятельно) будет заполнена автоматически. Если использовать [прямую ссылку](agrd.io/report), данные нужно будет ввести вручную.
...
Вы можете поучаствовать в решении проблемы. Для этого вам необходимо понять, какой элемент или функция не дают странице корректно загрузиться.
...
3. Если вы выявили, какой фильтр «ломает» страницу, сообщите об этом разработчикам при заполнении формы (см. выше). Также желательно вернуть настройки по умолчанию;
...
2. Если это решило проблему, включите модуль и поочерёдно отключите опции Антитрекинга. Это делается для того, чтобы выявить, какие именно настройки «ломают» страницу;
