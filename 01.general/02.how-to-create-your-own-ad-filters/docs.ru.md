...
* `||example.org^$hls=/regex/options` – удалить сегменты, URL которых, или, в зависимости от опций, один из тэгов, соответствует регулярному выражению `regex`. Опции `options` изменяют поведение правила как описано ниже.
...
4. `$hls` правила не могут удалить теги, относящиеся ко всему плей-листу. В данном случае могут быть использованы правила `$replace` .
