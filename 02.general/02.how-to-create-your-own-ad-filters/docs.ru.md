...
Правила с модификатором `$removeparam` предназначены для того, чтобы убрать параметры отслеживания из URL страниц и эффективно расширить список параметров отслеживания для режима Антитрекинга. *Обратите внимание*, что подобные правила применяются только к запросам типа `GET`.
...
> **Обратите внимание**, что блокирующее правило с параметром `$removeparam` должно содержать как минимум несколько уточнённых параметров. Например, правило вроде `example.com$removeparam` будет считаться неверным и применяться не будет. 
...
> **Важно:** избегайте специальных символов вроде `,`, `/` и `$` в регулярных выражениях. Используйте для этого: (`\`). Например, “запятая” будет в этом случае выглядеть так: `\,`.
...
С этими правилами Click ID компаний Google, Yandex и Facebook будут удалены, но запросы к `example.com` не будут удалены из Google Click ID. Например: `http://google.com/page?gclid=1&fbclid=2&yclid=3` будет преобразован в `http://google.com/page`, но `http://example.com/page?gclid=1&fbclid=2&yclid=3` будет преобразован в `http://example.com/page?gclid=1`.
