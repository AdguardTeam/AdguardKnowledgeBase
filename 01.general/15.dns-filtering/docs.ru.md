...
DNS-фильтрация может быть разложена на две основные функции: зашифровывать DNS-трафик и перенаправлять его на DNS-сервер, а также блокировать определённые домены, применяя локальные DNS-фильтры.
