...
To do so, enter and execute the following commands via Terminal: 
...
* Stop the process which is being&nbsp;run under the user (not the one run under root). Confirm it in the dialogue window.
