...
Данный модификатор упрощает создание правил, запрещющих определённое поведение по умолчанию при наличии исключений в статических фильтрах. 
...
Данное правило фильтрации блокирует на сайтах `a.com` и `b.com` все внешние (third-party) скрипты, кроме скриптов с доменов `x.com` и `y.com`.

**Обратите внимание, что [модификатор `$domain`](#domain-modifier) является обязательным!**

> **AdGuard для Safari and AdGuard для iOS** конвертируют правило с $denyallow в блокирующее правило и дополнительные правила исключения:
>
> Например, правило ```*$script,domain=a.com|~b.com,denyallow=x.com|y.com``` будет конвертировано в:
> ```
