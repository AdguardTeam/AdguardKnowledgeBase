...
Сейчас большинство сайтов работают через HTTPS — защищённый веб-протокол. Так сложилось, что рекламщики тоже не остаются в стороне и активно пользуются этим протоколом, поэтому [HTTPS-фильтрация](https://kb.adguard.com/ru/general/https-filtering) крайне важна для современного блокировщика рекламы. 
...
1) Перейдите на главный экран приложения и нажмите *Включить* рядом с сообщением *HTTPS-фильтрация выключена* (оно будет подсвечено оранжевым, если требуется установка сертификата).

2) Появится новый экран, нажмите кнопку *Далее*, затем ещё раз *Далее* и *Сохраните его сейчас*, когда вам будет предложено загрузить сертификат AdGuard.

3) После окончания загрузки сертификата вы увидите новый экран. Нажмите кнопку *Откройте настройки безопасности*.

4) Это вызовет переход в системные настройки. Прокрутите экран вниз до раздела *Дополнительно*, перейдите в него и далее в раздел *Шифрование и учётные данные* (внимание: название разделов может отличаться в зависимости от устройства — главное, выбрать дополнительные настройки безопасности).

5) Нажмите кнопку *Установка сертификатов* и затем кнопку *Сертификат центра сертификации*.

6) Появится окно с предупреждением. Прочитайте его и нажмите *Всё равно установить*, чтобы продолжить.
...
8) Сертификат AdGuard успешно установлен и HTTPS-фильтрация работает! 

>Если вы обновляете систему с Android 10 до Android 11, есть вероятность, что установленный ранее автоматически сертификат будет принят системой. В противном случае, вы увидите на главном экране сообщение *HTTPS-фильтрация выключена*, подсвеченное оранжевым. А значит, вам понадобится установить сертификат вручную по инструкции, приведённой выше. 

В случае возникновения проблем с установкой сертификата, прочитайте [эту статью](https://kb.adguard.com/ru/android/solving-problems/manual-certificate) или обратитесь в нашу <a href="mailto:support@adguard.com"службу поддержки</a>. 

**Мы хотим ещё раз подчеркнуть, что Блокировщик AdGuard будет работать и без фильтрации HTTPS. Однако он не сможет полноценно фильтровать рекламу на сайтах, использующих протокол HTTPS (а количество таких сайтов постоянно растёт). Поэтому, если вы хотите использовать все возможности приложения и обеспечить наилучшее качество и высокую скорость фильтрации рекламы, мы настоятельно рекомендуем вам активировать HTTPS-фильтрацию.** 
