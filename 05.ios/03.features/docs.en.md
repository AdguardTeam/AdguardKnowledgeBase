...
#### DNS requests log
DNS requests log is another useful feature of AdGuard Pro. The log contains the full list of processed DNS requests, from which you can choose any single one and add it to the Blocklist it on the spot, or add a previously blocked one to the Allowlist. This feature allows for wide choice of options to adjust the Blocklist and Allowlist as well as lets you track all requests in real time. To access it, tap on the stats tab in the app's main menu.
