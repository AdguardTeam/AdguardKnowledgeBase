...
##### Синтаксис
* `||example.org^$hls=urlpattern` – удалить сегменты чей URL соответствуют паттерну `urlpattern`. Паттерн работает так же, как в базовых URL правилах, однако символы `/`, `$` и `,` в составе `urlpattern` необходимо экранировать с помощью `\`.
* `||example.org^$hls=/regex/options` – удалить сегменты, чей URL, или, в зависимости от опций, один из тэгов, соответствует регулярному выражению `regex`. Опции `options` модифицируют поведение правила как показано ниже.

Доступные значения `options`:
* `t` – вместо URL сегмента, тестировать тэги сегмента на соответствие регулярному выражению. Сегмент с соответствующим тэгом будет удален.
...
* `@@||example.org^$hls` – отключить все `$hls` правила для ответов на запросы к URL, соответствующим паттерну `||example.org^`.
* `@@||example.org^$hls=text` – отключить все `$hls` правила, у которых значение модификатора `hls` равно `text`, для ответов на запросы к URL, соответствующим паттерну `||example.org^`.
* `$hls` также можно отключить с помощью правил-исключений с модификаторами `$document`, `$content` и `$urlblock`.

##### Ограничения
* `$hls` правила разрешены только в доверенных фильтрах.
* `$hls` не могут иметь других модификаторов, кроме `$domain`, `$third-party`, `$app`, `$important`, `$match-case`, и `$xmlhttprequest`.
* `$hls` правила применимы только к HLS плейлистам, т.е. к тексту в кодировке UTF-8, начинающемуся со строки `#EXTM3U`. Никакие другие ответы не будут модифицированы `$hls` правилами.
* `$hls` правила не будут применены к ответам размером больше 3 МБ.

##### Замечания
Если несколько `$hls` правил соответствуют одному и тому же запросу, их эффект суммируется.
...
* `||example.org^$hls=/#UPLYNK-SEGMENT:.*\,ad/t` — удалить все сегменты с соответствующим тэгом.

##### О формате HLS плейлистов

Краткий пересказ [спецификации](https://datatracker.ietf.org/doc/html/rfc8216):
1. HLS плейлист -- это набор текстовых строк.
2. Строка может быть пустой, быть комментарием (начинается с `#`), тэгом (начинается с `#` и имени тэга) или URL.
3. URL-строка называется сегментом.
4. Тэг может относиться к одному сегменту, т.е. к первой URL-строке, следующей после данного тэга,
   ко всем последующим сегментами (до тех пор пока не встретится тэг с тем же именем), или ко всему плейлисту.

Замечания, касающиеся `$hls` правил:
1. Когда какой-либо сегмент удаляется, все тэги, относящиеся только к этому сегменту, тоже удаляются.
2. Тэги, относящиеся к нескольким сегментам удаляются, если все эти сегменты были удалены.
3. `$hls` правила распознают все тэги, описанные в RFC, а также несколько распространненых нестандартны тегов. Все остальные строки, начинающиеся с `#`, считаются комментариями, и остаются нетронутыми.
4. `$hls` правила не могут удалить теги, относящиеся ко всему плей-листу. `$replace` правила могут быть использованы в данным случае.

Пример работы `$hls` правил:
<details>
...
`$redirect` — это модификатор для [базовых правил фильтрации](https://kb.adguard.com/ru/general/how-to-create-your-own-ad-filters#basic-rules), так что правила с этим модификатором поддерживают все другие базовые модификаторы, такие, как `$domain`, `$third-party`, `$script` и т.д.
