...
* `@@||example.org^$hls` – отключить все правила `$hls` для ответов на запросы к URL, соответствующим паттерну `||example.org^`.
* `@@||example.org^$hls=text` – отключить все правила `$hls`, у которых значение модификатора `hls` равно `text`, для ответов на запросы к URL, соответствующим паттерну `||example.org^`.
...
3. Правила `$hls` распознают все тэги, описанные в RFC, а также несколько распространенных нестандартных тегов. Все остальные строки, начинающиеся с `#`, считаются комментариями, и остаются нетронутыми.
